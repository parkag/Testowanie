 ...
 *
 * Note that it is an invariant that getLeft(d) &le; getRight(d) for all
 * dimensions d
 *
...
public class Hypercube implements IHypercube {
     ...
     * @exception IllegalArgumentException if n is < 2 or if the arrays differ
     * in length.
     */
    ...
    /**
     * Convenience method to construct a 2-dimensional Hypercube from two
     * points.
     * <p>
     * Note that the order of these parameters is at odds with what would be
     * found in RectangularRegion.
     * <p>
     *
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof IHypercube) {
            ...
            for (int i = 1; i <= dimension; i++) {
                ...
                if (!FloatingPoint.same(x, lows[i - 1])) {
                    return false;
                }

                x = ihc.getRight(i);
                if (!FloatingPoint.same(x, highs[i - 1])) {
                    return false;
                }
            }
    ...
    /**
     * Determine intersection among all point coordinates (in raw, optimized
     * form).
     *
     * @param rawPoint array of the coordinates of the target
     * {@link IMultiPoint}
     */
     ...
     *
     * @param p check for intersection among all coordinate dimensions for this
     * {@link IMultiPoint}.
     */
     ...
     * -INF or +INF is present, then we can take care to ensure proper
     * containment even in face of boundless dimensions.
     *
    ...
    public boolean intersects(IHypercube h) throws IllegalArgumentException {
        ...
        for (int i = 1; i <= dimension; i++) {
            ...
            if (innerRight < getLeft(i)) {
                continue;
            }
            if (innerLeft > getRight(i)) {
                continue;
            }
