...
public class TwoDLineSegment implements ILineSegment, IMultiLineSegment {
    ...
    /**
     * Sign of Slope. Needed to deal with the 3 situations when determining
     * pointOnRight. Assume positive
     */
     ...
     *
     * Method not actually used by any algorithms (yet). Returns one of:
     * <ul>
     ...
     * <li>COINCIDENT -- the two line segments intersect each other multiple
     * times because they are parallel.
     * </ul>
     *
     ...
     *
     * If no such intersection, return null. If null is returned, you may have
     * to invoke intersectionType to discover the reason.
     */
    ...
    /**
     * Given the line segment in its reverse-directed orientation (from lower to
     * upper point), determine if point p is to the right of the line, as you
     * head from lower to upper.
     *
     * If it is exactly on the line, then we return false.
     *
     * Note that we use the following inverted equation for lines to make the
     * determination. The equation can easily be confirmed, but care must be
     * taken based upon the sign of the slope (since we are dividing by m).
     *
     * <pre>
     *                 (y2 - y1)
     *    0 = y - y1 - --------- (x - x1)
     ...
     * </pre>
     *
     * Since this operation is SO COMMON, we use a standard trick to remove the
     * IF statements. Specifically, if m is positive, then we check if res &lt;
     * 0 but if m is negative, then we check if -res &lt; 0. Thus we also need
     * to store the SIGN of the slope with each segment.
     *
    ...
    /**
     * Given the line segment in its reverse-directed orientation (from lower to
     * upper point), determine if point p is to the left of the line, as you
     * head from lower to upper
     *
     * If it is exactly on the line, then we return false.
     *
     * See {@link #pointOnRight(IPoint)} for the mathematical explanation for
     * these equations, as well as importance of
     * <code>sign</code>.
     *
     * @param p
     */
