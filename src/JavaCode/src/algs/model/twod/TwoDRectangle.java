 ...
 * <p>
 * Note that the rectangle could be infinite in none, one, or two of these
 * dimensions by having any of its coordinates set to Double.NEGATIVE_INFINITY
 * or Double.POSITIVE_INFINITY. A rectangle could be one-dimensional (if either
 * x1==x2 or y1==y2) or zero-dimensional (if both x1==x2 and y1==y2).
 * <p>
 * RectangularRegion is (slightly) incompatible with IHypercube because
 * rectangles are commonly referred to as (left, bottom, right, top). As you can
 * see, the dimensions are intermixed. IHypercube expects a number of
 * dimensions, and to keep things straight they are reported in order: here this
 * means it would be (left, right) (bottom, top). In order to avoid confusion,
 * we make no attempt to have this class conform to both IRectangle and
 * IHypercube.
 * <p>
...
public class TwoDRectangle implements IRectangle {
    ...
    /**
     * Determines intersection of the given point within the closed Rectangular
     * region.
     *
    ...
    /**
     * Determines containment of the given rectangle within the closed
     * Rectangular region.
     * <p>
