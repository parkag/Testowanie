...
/**
 * Defines a standard <b>inorder</b> traversal of the KDTree and enables
 * subclasses to provide specialized method to take action at each node of the
 * tree.
 * <p>
 * By referencing the tree, the traversal can be reused even when the tree is
 * altered. Note, however, that altering the tree structure <em>during</em> a
 * traversal will lead to non-determined behavior.
 * <p>
 * This traversal supports both traversals starting at the root, and those
 * starting at arbitrary points in the hierarchy of the tree.
 *
...
public abstract class KDTraversal implements IVisitKDNode {
    ...
    /**
     * During a regular traversal, drain is not invoked. To avoid subclasses
     * mistakenly thinking they must provide this method, we implement here and
     * mark as final to prevent that mistake. Since it is never called, this
     * does nothing.
     */
