...
/**
 * Represents a node in the KD-tree that partitions the space by means of a
 * plane that splits the hyperspace into an "above" and a "below" based upon
 * orientation.
 * <p>
 ...
 * <p>
 * Each dimensional node stores a Region for which it is responsible. Note that
 * only the Root is boundless in all directions (i.e., [-INF, +INF, -INF, +INF].
 * Thereafter, child nodes are able to maintain their regions. When a
 * dimensional node is inserted into the tree, its SPACE is properly updated
 * based upon the parent, and dimension to which it is being added.
 * <p>
 * Because the original MultiPoint may have lots of dimensions, we store the
 * values here for much faster access then simply accessing values through its
 * methods.
 *
...
public class DimensionalNode {
    ...
    /**
     * Dimensional-coordinate is passed in as the value, together with its
     * dimension.
     * <p>
     ...
     * <p>
     * Will only be true for root node in KD-tree or newly instantiated nodes
     * that have yet to be added to a KD tree.
     */
     ...
     * <p>
     * In short, this acts as a factory for the nodes in the next level of the
     * tree.
     *
     ...
     * <p>
     * Speedup occurs when an entire range is found to exist within the target
     * space, thus all points in the subtree rooted here can be drained and
     * added.
     *
     * @param space non-null region within which search occurs.
     * @param results non-null ArrayList<IMultiPoint> into which located points
     * are inserted.
    ...
    /**
     * Locate all points within the KDTree that fall within the given hyperspace
     * and use given visitor as the computation to perform on that node.
     * <p>
     * Speedup occurs when an entire range is found to exist within the target
     * space, thus all points in the subtree rooted here can be added.
     * <p>
     * If we are a leaf, then report the point if it is contained within space.
     * Perhaps our region is wholly contained within space. If so, then we can
     * safely add self and all points from our left and right subtrees.
     * Otherwise, we have to do some computation to see whether we go down left
     * and/or right subtrees.
     *
    ...
    /**
     * Helper method to visit all descendant nodes in the tree rooted at given
     * node.
     */
    ...
    int height() {
        int maxHeight = 1;
        if (below != null) {
            int h = 1 + below.height();
            if (h > maxHeight) {
                maxHeight = h;
            }
        }
        if (above != null) {
            int h = 1 + above.height();
            if (h > maxHeight) {
                maxHeight = h;
            }
        }
     ...
     * <p>
     * This method is used only by the KDFactory which needs to maintain the
     * regions properly because its algorithm constructed the KDTree from the
     * bottom up. It is marked as package private to ensure the integrity of the
     * tree.
     * <p>
     *
