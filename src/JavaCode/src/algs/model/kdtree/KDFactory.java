 ...
 * <p>
 * Note that we take care to construct the associated {@link Hypercube} regions
 * with each node to ensure the integrity of the regions; without these regions
 * being properly set, there is no way to "drain" the children of a subtree when
 * a query wholly contains a subtree's region.
 *
...
public class KDFactory {
     ...
     * <p>
     * Uses 1-based access for ease of programming, so always is one larger than
     * necessary.
     */
     ...
     * <p>
     * All points must have the same dimensionality, otherwise strange behavior
     * may occur. Also, this method is not re-entrant, since the comparators
     * array is regenerated upon each invocation, thus we mark the method as
     * 'synchronized'.
     *
     ...
     *
     * If underlying objects in points[] already implement IMultiPoint, then
     * good. Otherwise a new array of TwoDPoint objects is constructed to be
     * passed into the generate method on an IMultiPoint[] array (where the
     * values for these TwoDPoints are extracted from the X and Y coordinate
     * values of the IPoint objects).
     *
     * @param points
     */
