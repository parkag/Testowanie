 ...
 * <p>
 * Stores a set of points against which one can execute 2-dimensional range
 * queries against a rectangle D whose domain is defined by a rectangle
 * D=[x1,x2] x [y1,y2]. Note that the rectangle could be infinite in none, one,
 * or two of these dimensions by having any of its coordinates set to
 * Double.NEGATIVE_INFINITY or Double.POSITIVE_INFINITY. A rectangle could be
 * one-dimensional (if either x1==x2 or y1==y2) or zero-dimensional (if both
 * x1==x2 and y1==y2).
 * <p>
 * Note that the example above is for Double values; the node values stored
 * could be of any type that implements Comparable.
 * <p>
...
public class KDTree {
    ...
    /**
     * Helper method to always determine the next dimensionality given an
     * integer representing the specified dimension in the KDTree.
     *
     * Used in test cases.
     *
    ...
    /**
     * Return the parent of the point IF the point were to be inserted into the
     * kd-tree.
     *
     * Returns null only if the tree is empty to begin with (i.e., there are no
     * parents).
     *
     * Note that you will have to inspect the dimension for the DimensionalNode
     * to determine if this is a Below or an Above parent.
     */
     ...
     *
     * Use with Caution! One issue that may cause problems is when the new root
     * does not have 'boundless' space associated with it, so the initial
     * infinite region is propagated throughout the tree whenever the root is
     * set.
     *
     ...
     * <p>
     * Only returns null if the tree was initially empty. Otherwise, must return
     * some point that belongs to the tree.
     * <p>
     * If tree is empty or if the target is
     * <code>null</code> then
    ...
    /**
     * Locate all points within the KDTree that fall within the given
     * IHypercube.
     *
     ...
     * @param visitor non-null IVisitTwoDNode visitor that contains behavior to
     * process at each discovered point within the space.
